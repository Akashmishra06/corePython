...
class R_40_40_30(baseAlgoLogic):
    ...
    def backtest(self, stockName, startDate, endDate):
        ...
        stocks = ["ADANIENT", "ADANIPORTS", "APOLLOHOSP", "ASIANPAINT", "AXISBANK", "BAJFINANCE", "BAJAJFINSV", "BPCL", "BHARTIARTL", "BRITANNIA", "CIPLA", "COALINDIA", "DIVISLAB", "DRREDDY", "EICHERMOT", "GRASIM", "HCLTECH", "HDFCBANK", "HDFCLIFE", "HEROMOTOCO", "HINDALCO", "HINDUNILVR", "ICICIBANK", "ITC", "INDUSINDBK", "INFY", "JSWSTEEL", "KOTAKBANK", "LT", "M&M", "MARUTI", "NTPC", "NESTLEIND", "ONGC", "POWERGRID", "RELIANCE", "SBILIFE", "SBIN", "SUNPHARMA", "TCS", "TATACONSUM", "TATAMOTORS", "TATASTEEL", "TECHM", "TITAN", "UPL", "ULTRACEMCO", "WIPRO"]
        ...
        for timeData in df_dict['ADANIENT'].index:
            for stock in stocks:
                ...
                if lastIndexTimeData in df_dict[stock].index:
                    if (stock_openPnl.empty):
                        entry_price = df_dict[stock].at[lastIndexTimeData, "c"]
                        stockAlgoLogic.entryOrder(entry_price, stock, (amountPerTrade // entry_price), "SELL")
                ...
                for index, row in stock_openPnl.iterrows():
                    if lastIndexTimeData in df_dict[stock].index:
                        if index in stock_openPnl.index:
                            ...
                            logger.info(f"stopLoss- Datetime: {stockAlgoLogic.humanTime}\tStock: {stock}\tClose: {df_dict[stock].at[lastIndexTimeData, 'c']}")
