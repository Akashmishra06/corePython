...
class DowTheory(baseAlgoLogic):
    ...
    def backtest(self, stockName, startDate, endDate):
        ...
        FirstThirtyTrigger = False
        FirstSextyTrigger = False
        FirstSextyTriggerPrice = None
        secondThirtyTrigger = False
        ...
        for timeData in df.index:
            ...
            for index, row in stockAlgoLogic.openPnl.iterrows():
                if lastIndexTimeData in df.index:
                    if (df.at[lastIndexTimeData, "rsi"] < 30) and (df.at[lastIndexTimeData, "c"] > row['EntryPrice']):
            ...
            if (lastIndexTimeData in df.index) & (stockAlgoLogic.openPnl.empty):
                if (df.at[lastIndexTimeData, "rsi"] < 30) and (df.at[lastIndexTimeData, "prev_rsi"] > 30):
                    FirstThirtyTrigger = True
                    FirstThirtyTriggerPrice = df.at[lastIndexTimeData, "c"]
                elif FirstThirtyTrigger and (df.at[lastIndexTimeData, "rsi"] > 60) and (df.at[lastIndexTimeData, "prev_rsi"] < 60):
                    FirstSextyTrigger = True
                    FirstThirtyTrigger = False
                    FirstSextyTriggerPrice = df.at[lastIndexTimeData, "c"]
                elif FirstSextyTrigger and (df.at[lastIndexTimeData, "rsi"] < 30) and (df.at[lastIndexTimeData, "prev_rsi"] > 30):
                    secondThirtyTrigger = True
                    FirstSextyTrigger = False
                    secondThirtyTriggerPrice = df.at[lastIndexTimeData, "c"]
                elif secondThirtyTrigger and (FirstSextyTriggerPrice < df.at[lastIndexTimeData, "c"]) and (df.at[lastIndexTimeData, "rsi"] > 60) and (df.at[lastIndexTimeData, "prev_rsi"] < 60):
                    secondSextyTrigger = True
                    secondThirtyTrigger = False
                    secondSextyTriggerPrice = df.at[lastIndexTimeData, "c"]
                    entry_price = df.at[lastIndexTimeData, "c"]
                    stockAlgoLogic.entryOrder(entry_price, stockName,  (amountPerTrade//entry_price), "BUY", {"stoploss": })
...
if __name__ == "__main__":
    ...
    portfolio = createPortfolio("/root/equityResearch/stocksList/nifty50.md", 4)
