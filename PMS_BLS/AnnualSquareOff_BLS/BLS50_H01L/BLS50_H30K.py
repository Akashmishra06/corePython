...
class BLS50_H30K_Annual_SquareOff(baseAlgoLogic):
    def runBacktest(self, portfolio, startDate, endDate):
        if self.strategyName != "BLS50_H30K_Annual_SquareOff":
    ...
    def backtest(self, stockName, startDate, endDate):
        ...
        def process_stock(stock, startTimeEpoch, endTimeEpoch, df_dict):
            ...
            if df is not None:
                ...
                df["AfterSqureOffEnteryClose"] = df['c'].shift(-2)
            ...
            for stock in stocks:
                ...
                for index, row in stock_openPnl.iterrows():
                    if lastIndexTimeData in df_dict[stock].index:
                        if index in stock_openPnl.index and breakEvenPrice[stock] != 0:

                            if df_dict[stock].at[lastIndexTimeData, "rsi"] < 30 and df_dict[stock].at[lastIndexTimeData, "c"] < breakEvenPrice[stock]:
                                breakEven[stock] = True

                            if breakEven[stock] == True and df_dict[stock].at[lastIndexTimeData, "c"] > breakEvenPrice[stock]:
                                exitType = "BrekevenExit"
                                breakEven[stock] = False
                                breakEvenPrice[stock] = 0
                                ...
                                logger.info(f"{nowTotalTrades}, TotalTradeCanCome:- {TotalTradeCanCome},BrekevenExit and breakEvenPrice:{breakEvenPrice[stock]}- Datetime: {stockAlgoLogic.humanTime}, Stock: {stock}, pnll:{pnll}, exitPrice: {df_dict[stock].at[lastIndexTimeData, 'c']}")

                            elif breakEven[stock] == False and df_dict[stock].at[lastIndexTimeData, "rsi"] < 30 and df_dict[stock].at[lastIndexTimeData, "c"] > breakEvenPrice[stock]:
                                ...
                                exitType = "annualSquareOffLossMakingTrades"
                                pnll = (row['EntryPrice'] - df_dict[stock].at[lastIndexTimeData, "c"]) * row['Quantity']
                                stockAlgoLogic.exitOrder(index, exitType, df_dict[stock].at[lastIndexTimeData, "c"])
                                nowTotalTrades = len(stockAlgoLogic.openPnl)
                                logger.info(f"{nowTotalTrades}, TotalTradeCanCome:- {TotalTradeCanCome},annualSquareOffLossMakingTrades- Datetime: {stockAlgoLogic.humanTime}, Stock: {stock}, pnll: {pnll}, exitPrice: {df_dict[stock].at[lastIndexTimeData, 'c']}")

                                entry_price = df_dict[stock].at[lastIndexTimeData, "AfterSqureOffEnteryClose"]
                                nowTotalTrades = nowTotalTrades + 1
                                stockAlgoLogic.entryOrder(entry_price, stock, row['Quantity'], "BUY")
                                logger.info(f"{nowTotalTrades}, TotalTradeCanCome:-{TotalTradeCanCome}, annualSquareOffLastEntry-{stock}- Datetime: {stockAlgoLogic.humanTime}, entryPrice: {entry_price}")
...
if __name__ == "__main__":
    ...
    devName = "AK"
    strategyName = "BLS50_H30K_Annual_SquareOff"
    version = "v1"
    ...
    algoLogicObj = BLS50_H30K_Annual_SquareOff(devName, strategyName, version)
