...
df = pd.read_csv('/root/equityResearch/PMS_BLS/AnnualSquareOff_BLS/BLS50_V01L/dfto_.csv')
...
# Now create the table-style report in another_report.txt
table_report_file = os.path.join(root_dir, 'another_report.txt')

# Get the list of years sorted
years = sorted(yearly_report.keys())

# Create the header for the table (categories)
table_header = "\t".join(["Year"] + categories)

# Open the file and write the table
with open(table_report_file, 'w') as table_report:
    table_report.write(table_header + "\n")
    
    # Write each year's data
    for year in years:
        row = [str(year)]  # Start with the year
        for category in categories:
            row.append(f"{yearly_report[year].get(category, 0)}")  # Add PnL value for each category
        table_report.write("\t".join(row) + "\n")

print("Report generated: 'another_report.txt'.")
