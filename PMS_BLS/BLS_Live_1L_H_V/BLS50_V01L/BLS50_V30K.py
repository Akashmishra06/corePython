...
class BLS50_V30K(baseAlgoLogic):
    def runBacktest(self, portfolio, startDate, endDate):
        if self.strategyName != "BLS50_V30K":
    ...
    def backtest(self, stockName, startDate, endDate):
        ...
        def process_stock(stock, startTimeEpoch, endTimeEpoch, df_dict):
            ...
            if df is not None:
                ...
                results = taa.supertrend(df["h"], df["l"], df["c"], length=10, multiplier=3.0)
        ...
        for timeData in df_dict['ADANIENT'].index:
            for stock in stocks:
                ...
                if lastIndexTimeData in df_dict[stock].index:
                    nowTotalTrades = len(stockAlgoLogic.openPnl)
                    if df_dict[stock].at[lastIndexTimeData, "rsi"] > 60 and df_dict[stock].at[lastIndexTimeData, "Supertrend"] == -1 and (stock_openPnl.empty) and nowTotalTrades < TotalTradeCanCome:
...
if __name__ == "__main__":
    ...
    algoLogicObj = BLS50_V30K(devName, strategyName, version)
