...
# BLS01_V50
class Vertical_50_100(baseAlgoLogic):
    def runBacktest(self, portfolio, startDate, endDate):
        if self.strategyName != "Vertical_50_100":
    ...
    def backtest(self, stockName, startDate, endDate):
        ...
        df = {}
        directory = "/root/akashBacktestAlgos/annualWeeklyDaily/Horizontal50/BacktestResults/NA_Vertical50_v1/1/CandleData"
        ...
        # Iterate over timeData in 'AARTIIND' DataFrame
        for timeData in df.get('AARTIIND', pd.DataFrame()).index:  # Use get to prevent KeyError if 'AARTIIND' doesn't exist
            for stock in df.keys():
                # Ensure that 'AARTIIND' DataFrame exists
                ...
                # Check if `lastIndexTimeData` is not None and access 'datetime' for the stock
                if lastIndexTimeData is not None:
                    try:
                        ...
                        stockAlgoLogic.humanTime = None  # Assign None or handle appropriately
                    except Exception as e:
                        print(f"An unexpected error occurred: {e}")
                        stockAlgoLogic.humanTime = None  # Assign None or handle appropriately

                # Check if lastIndexTimeData is in the stock's DataFrame index
                    ...
                    if lastIndexTimeData in df[stock].index:
                        if index in stock_openPnl.index:
                            if df[stock].at[lastIndexTimeData, "timeUp"] == "timeUp":
                                ...
                                # output_string = f"{nowTotalTrades},TotalTradeCanCome:-{TotalTradeCanCome}, {df[stock].at[lastIndexTimeData, 'datetime']}, lastDay: {stock}, PnL:{PnL}\n"
                                ...
                                PnL = ((abs(df[stock].at[lastIndexTimeData, "c"] - row['EntryPrice']) * row['Quantity']))
                ...
                if lastIndexTimeData is not None:
                    if ProfitAmount > 100000:
                        ...
                        nowTotalTrades = len(stockAlgoLogic.openPnl)
                        output_string = f"{nowTotalTrades},TotalTradeCanCome:-{TotalTradeCanCome}, {df[stock].at[lastIndexTimeData, 'datetime']}, LossAmountIncrease: {stock}, LossAmount: {LossAmount}\n"
                        with open('reposrrrt.txt', 'a') as file:
                            file.write(output_string)
...
if __name__ == "__main__":
    ...
    devName = "NA"
    strategyName = "Vertical_50_100"
    version = "v1"

    startDate = datetime(2019, 1, 1, 9, 15)
    endDate = datetime(2024, 12, 31, 15, 30)

    portfolio = createPortfolio("/root/akashBacktestAlgos/stocksList/Fno_173.md",1)

    algoLogicObj = Vertical_50_100(devName, strategyName, version)
