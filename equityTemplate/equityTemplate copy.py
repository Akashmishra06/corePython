...
class EquityBacktest(baseAlgoLogic):
    ...
    def backtest(self, stockName, startDate, endDate):
        ...
        def process_stock(stock, startTimeEpoch, endTimeEpoch, fileDir, df_dict):
            df = getEquityBacktestData(stock, startTimeEpoch - (86400 * 500), endTimeEpoch, "5Min")
            ...
            if df is not None:
                ...
                df_dict[stock] = df
                df.to_csv(f"{fileDir['backtestResultsCandleData']}{stock}_df.csv")
                print(f"Finished processing {stock}")

        def process_stocks_in_parallel(stocks, startTimeEpoch, endTimeEpoch, fileDir):
            threads = []
            df_dict = {}

            for stock in stocks:
                thread = threading.Thread(target=process_stock, args=(stock, startTimeEpoch, endTimeEpoch, fileDir, df_dict))
        ...
        fileDir = {'backtestResultsCandleData': './data/'}

        df_dict = process_stocks_in_parallel(stocks, startTimeEpoch, endTimeEpoch, fileDir)
