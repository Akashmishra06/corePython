...
class oldTempCheck(baseAlgoLogic):
    ...
    def backtest(self, stockName, startDate, endDate):
        ...
        # df['macd'] = talib.MACD(df["c"], fastperiod=12, slowperiod=26, signalperiod=9)
        ...
        for timeData in df.index:
            ...
            for index, row in stockAlgoLogic.openPnl.iterrows():
                if lastIndexTimeData in df.index:
                    ...
                    if (df.at[lastIndexTimeData, "rsi"] < 60) and (df.at[lastIndexTimeData, "c"] > row['EntryPrice']) and (df.at[lastIndexTimeData, "ema9"] < df.at[lastIndexTimeData, "prev_ema"]):
...
if __name__ == "__main__":
    ...
    portfolio = createPortfolio("/root/equityResearch/nifty50/stocks.txt", 4)
