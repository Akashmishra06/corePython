...
class oldTempCheck(baseAlgoLogic):
    def runBacktest(self, portfolio, startDate, endDate):
        if self.strategyName != "oldTempCheck":
        ...
        for timeData in df.index:
            ...
            if (lastIndexTimeData in df.index) & (stockAlgoLogic.openPnl.empty):
                ...
                if (df.at[lastIndexTimeData, "rsi"] < 30) and (df.at[lastIndexTimeData, "prev_rsi"] > 30):
                    oneEntryTrigger = True

                elif oneEntryTrigger and (df.at[lastIndexTimeData, "rsi"] > 40) and (df.at[lastIndexTimeData, "prev_rsi"] < 40):
                    oneEntryTrigger = False
                    stoploss = df.at[lastIndexTimeData, "rsi"]
...
if __name__ == "__main__":
    ...
    algoLogicObj = oldTempCheck(devName, strategyName, version)
