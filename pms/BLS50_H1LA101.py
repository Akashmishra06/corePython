...
class BLS50_H1LA101(baseAlgoLogic):
    def runBacktest(self, portfolio, startDate, endDate):
        if self.strategyName != "BLS50_H1LA101":
    ...
    def backtest(self, stockName, startDate, endDate):
        ...
        def process_stock(stock, startTimeEpoch, endTimeEpoch, df_dict):
            ...
            df_1 = getFnoBacktestData("NIFTY 50", startTimeEpoch - (86400 * 500), endTimeEpoch, "D")
            df_1["niftyRsi"] = talib.RSI(df_1["c"], timeperiod=14)
            df_1.index = df_1.index + 33300
            df_1 = df_1[df_1.index > startTimeEpoch]
            ...
            if df is not None:
                ...
                df = pd.merge(df, df_1[['datetime', 'niftyRsi']], on='datetime', how='left')
                df_dict[stock] = df
        ...
        stocks = [
            "COALINDIA", "AARTIIND", "ADANIENT", "BAJAJ-AUTO", "HDFCBANK", "ABB", "ABBOTINDIA","ABCAPITAL", "ABFRL", "ACC", "ADANIPORTS", "AMBUJACEM", "APOLLOHOSP", "ASHOKLEY", "ASIANPAINT", "ASTRAL", "AUROPHARMA", "AXISBANK", "BAJAJFINSV", "BAJFINANCE", 
            "BALKRISIND", "BALRAMCHIN", "BANDHANBNK", "BANKBARODA", "BATAINDIA", "BEL", "BERGEPAINT", "BHARATFORG", "BHARTIARTL", "BHEL", "BIOCON", "BOSCHLTD", "BPCL", "BRITANNIA", "BSOFT", 
            "CANBK", "CANFINHOME", "CHAMBLFERT", "CHOLAFIN", "CIPLA", "COFORGE", "COLPAL", "CONCOR", "COROMANDEL", "CROMPTON", "CUB", "CUMMINSIND", "DABUR", "DALBHARAT", "DEEPAKNTR", "DIVISLAB", 
            "DIXON", "DLF", "DRREDDY", "EICHERMOT", "ESCORTS", "EXIDEIND", "FEDERALBNK", "GAIL", "GLENMARK", "GMRINFRA", "GNFC", "GODREJCP", "GODREJPROP", "GRANULES", "GRASIM", "GUJGASLTD", 
            "HAVELLS", "HCLTECH", "HDFCAMC", "HDFCLIFE", "HEROMOTOCO", "HINDALCO", "HINDCOPPER", "HINDPETRO", "HINDUNILVR", "HINDZINC", "ICICIBANK", "ICICIGI", "ICICIPRULI", "IDFC", 
            "IDFCFIRSTB", "IGL", "INDHOTEL", "INDIACEM", "INDIGO", "INDUSINDBK", "INDUSTOWER", "INFY", "IOC", "IPCALAB", "IRCTC", "ITC", "JINDALSTEL", "JKCEMENT", "JUBLFOOD", "KOTAKBANK", 
            "LALPATHLAB", "LAURUSLABS", "LICHSGFIN", "LT", "LTIM", "LTTS", "LUPIN", "M&M", "M&MFIN", "MANAPPURAM", "MARICO", "MARUTI", "MCX", "METROPOLIS", "MFSL", "MGL", "MOTHERSON", 
            "MPHASIS", "MUTHOOTFIN", "NAUKRI", "NAVINFLUOR", "NESTLEIND", "NATIONALUM", "NMDC", "NTPC", "OBEROIRLTY", "OFSS", "ONGC", "PAGEIND", "PEL", "PERSISTENT", "PETRONET", "PFC", "PIDILITIND", 
            "PIIND", "PNB", "POLYCAB", "POWERGRID", "PVRINOX", "RAMCOCEM", "RBLBANK", "RECLTD", "RELIANCE", "SAIL", "SBILIFE", "SBIN", "SHREECEM", "SHRIRAMFIN", "SIEMENS", "SYNGENE", "SUNPHARMA", "SUNTV", "TATACHEM", "TATACOMM", "TATACONSUM", "TATAMOTORS", "TATAPOWER", "TATASTEEL","TCS", "TECHM", "TORNTPHARM", "TRENT", "TVSMOTOR", "UBL", "ULTRACEMCO", "UPL", "VEDL", "VOLTAS", "WIPRO", "ZYDUSLIFE"
        ]
        ...
        rsiTargetExit = {stock: False for stock in stocks}
            ...
            for stock in stocks:
                ...
                for index, row in stock_openPnl.iterrows():
                    if lastIndexTimeData in df_dict[stock].index:
                        if index in stock_openPnl.index:
                            ...
                            if breakEven[stock] != True and df_dict[stock].at[lastIndexTimeData, "rsi"] < 30 and df_dict[stock].at[lastIndexTimeData, "c"] < row['EntryPrice']:
                                breakEven[stock] = True

                            elif breakEven[stock] == True and df_dict[stock].at[lastIndexTimeData, "c"] > row['EntryPrice']:
                                exitType = f"BrekevenExit, {row['Date']}, {df_dict[stock].at[lastIndexTimeData, 'datetime']}"
                                ...
                                logger.info(f"{nowTotalTrades}, TotalTradeCanCome:- {TotalTradeCanCome},BrekevenExit- Datetime: {df_dict[stock].at[lastIndexTimeData, 'datetime']}, Stock: {stock}, pnll:{pnll}, exitPrice: {df_dict[stock].at[lastIndexTimeData, 'c']}")

                            elif df_dict[stock].at[lastIndexTimeData, "rsi"] < 30 and df_dict[stock].at[lastIndexTimeData, "c"] > row['EntryPrice']:
                                rsiTargetExit[stock] = True
                            elif df_dict[stock].at[lastIndexTimeData, "rsi"] > 60:
                                rsiTargetExit[stock] = False
                            if rsiTargetExit[stock] == True and df_dict[stock].at[lastIndexTimeData, "niftyRsi"] < 50:
                                exitType = f"RsiTargetHit, {row['Date']}, {df_dict[stock].at[lastIndexTimeData, 'datetime']}"
                                pnll = (df_dict[stock].at[lastIndexTimeData, "c"] - row['EntryPrice']) * row['Quantity']
                                ProfitAmount = ProfitAmount + pnll
                                stockAlgoLogic.exitOrder(index, exitType, df_dict[stock].at[lastIndexTimeData, "c"])
                                nowTotalTrades = len(stockAlgoLogic.openPnl)
                                logger.info(f"{nowTotalTrades}, TotalTradeCanCome:- {TotalTradeCanCome},RsiTargetHit- Datetime: {df_dict[stock].at[lastIndexTimeData, 'datetime']}, Stock: {stock}, pnll: {pnll},ProfitAmount: {ProfitAmount}, exitPrice: {df_dict[stock].at[lastIndexTimeData, 'c']}")

                            if row['entryType'] == "two" and df_dict[stock].at[lastIndexTimeData, "c"] > (row['EntryPrice'] * 2):
                                exitType = f"DoubleTargetExit, {row['Date']}, {df_dict[stock].at[lastIndexTimeData, 'datetime']}"
                                pnll = (df_dict[stock].at[lastIndexTimeData, "c"] - row['EntryPrice']) * row['Quantity']
                                ProfitAmount = ProfitAmount + pnll
                                stockAlgoLogic.exitOrder(index, exitType, df_dict[stock].at[lastIndexTimeData, "c"])
                                nowTotalTrades = len(stockAlgoLogic.openPnl)
                                logger.info(f"{nowTotalTrades}, TotalTradeCanCome:- {TotalTradeCanCome},DoubleTargetExit- Datetime: {df_dict[stock].at[lastIndexTimeData, 'datetime']}, Stock: {stock}, pnll: {pnll},ProfitAmount: {ProfitAmount}, exitPrice: {df_dict[stock].at[lastIndexTimeData, 'c']}")

                if ProfitAmount > 100000:
                    ProfitAmount = ProfitAmount - 100000
                    TotalTradeCanCome = TotalTradeCanCome + 2
                    logger.info(f"{nowTotalTrades}, TotalTradeCanCome:- {TotalTradeCanCome}, Datetime: {df_dict[stock].at[lastIndexTimeData, 'datetime']},ProfitAmount:-{ProfitAmount}")
                ...
                if lastIndexTimeData in df_dict[stock].index:
                    ...
                    if df_dict[stock].at[lastIndexTimeData, "rsi"] > 60 and (stock_openPnl.empty) and nowTotalTrades < TotalTradeCanCome:
                        ...
                        nowTotalTrades = nowTotalTrades + 2
                        breakEven[stock] = False
                        stockAlgoLogic.entryOrder(entry_price, stock, (amountPerTrade // entry_price)//2, "BUY", {"entryType": "one", "Date": df_dict[stock].at[lastIndexTimeData, "datetime"]})
                        stockAlgoLogic.entryOrder(entry_price, stock, (amountPerTrade // entry_price)//2, "BUY", {"entryType": "two", "Date": df_dict[stock].at[lastIndexTimeData, "datetime"]})
                        logger.info(f"{nowTotalTrades}, TotalTradeCanCome:-{TotalTradeCanCome}, Entry-{stock}- Datetime: {df_dict[stock].at[lastIndexTimeData, 'datetime']}, entryPrice: {df_dict[stock].at[lastIndexTimeData, 'c']}")
        ...
        for index, row in stockAlgoLogic.openPnl.iterrows():
            if lastIndexTimeData in df_dict[stock].index:
                if index in stockAlgoLogic.openPnl.index:
                    exitType = f"TimeUp, {row['Date']}, {df_dict[stock].at[lastIndexTimeData, 'datetime']}"
...
if __name__ == "__main__":
    ...
    algoLogicObj = BLS50_H1LA101(devName, strategyName, version)
