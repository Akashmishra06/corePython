        ...
        for timeData in df_dict['ADANIENT'].index:
            for stock in stocks:
                ...
                for index, row in stock_openPnl.iterrows():
                    if lastIndexTimeData in df_dict[stock].index:
                        if index in stock_openPnl.index:
                            if df_dict[stock].at[lastIndexTimeData, "rsi"] < 40 and df_dict[stock].at[lastIndexTimeData, "c"] > row['EntryPrice']:
                ...
                if lastIndexTimeData in df_dict[stock].index:
                    ...
                    if df_dict[stock].at[lastIndexTimeData, "rsi"] > 40 and (stock_openPnl.empty) and nowTotalTrades < TotalTradeCanCome:
