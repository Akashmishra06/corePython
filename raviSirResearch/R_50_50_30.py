                ...
                for index, row in stock_openPnl.iterrows():
                    if lastIndexTimeData in df_dict[stock].index:
                        if index in stock_openPnl.index:
                            if df_dict[stock].at[lastIndexTimeData, "rsi"] < 50 and df_dict[stock].at[lastIndexTimeData, "c"] > row['EntryPrice']:
                                ...
                                exitType = "stopLoss"
                                stockAlgoLogic.exitOrder(index, exitType, df_dict[stock].at[lastIndexTimeData, "c"])
                                logger.info(f"nowTotalTrades:-{nowTotalTrades},stopLoss- Datetime: {stockAlgoLogic.humanTime}\tStock: {stock}\tClose: {df_dict[stock].at[lastIndexTimeData, 'c']}")

                if lastIndexTimeData in df_dict[stock].index:
                    nowTotalTrades = len(stockAlgoLogic.openPnl)
                    if df_dict[stock].at[lastIndexTimeData, "rsi"] > 50 and (stock_openPnl.empty) and nowTotalTrades < TotalTradeCanCome:
