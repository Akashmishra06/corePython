            ...
            for stock in stocks:
                ...
                for index, row in stock_openPnl.iterrows():
                    if lastIndexTimeData in df_dict[stock].index:
                        if index in stock_openPnl.index:
                            ...
                            if df_dict[stock].at[lastIndexTimeData, "rsi"] < 60 and df_dict[stock].at[lastIndexTimeData, "c"] > row['EntryPrice']:
                                ...
                                logger.info(f"ProfitAmount:{ProfitAmount},nowTotalTrades:-{nowTotalTrades},RsiTargetHit- Datetime: {stockAlgoLogic.humanTime}\tStock: {stock}\tClose: {df_dict[stock].at[lastIndexTimeData, 'c']}")

                            elif df_dict[stock].at[lastIndexTimeData, "rsi"] < 30:
                                exitType = "stopLoss"
                                LossAmount = LossAmount + (row['EntryPrice'] - df_dict[stock].at[lastIndexTimeData, "c"]) * row['Quantity']
                                stockAlgoLogic.exitOrder(index, exitType, df_dict[stock].at[lastIndexTimeData, "c"])
                                logger.info(f"LossAmount: {LossAmount},nowTotalTrades:-{nowTotalTrades},stopLoss- Datetime: {stockAlgoLogic.humanTime}\tStock: {stock}\tClose: {df_dict[stock].at[lastIndexTimeData, 'c']}")

                if ProfitAmount > 100000:
                    ProfitAmount = ProfitAmount - 100000
                    TotalTradeCanCome = TotalTradeCanCome + 1
                    logger.info(f"TotalTradeCanCome: {TotalTradeCanCome},ProfitAmount: {ProfitAmount},nowTotalTrades:-{nowTotalTrades},- Datetime: {stockAlgoLogic.humanTime}\tStock: {stock}\tClose: {df_dict[stock].at[lastIndexTimeData, 'c']}")

                if LossAmount > 100000:
                    LossAmount = LossAmount - 100000
                    TotalTradeCanCome = TotalTradeCanCome - 1
                    logger.info(f"TotalTradeCanCome: {TotalTradeCanCome},LossAmount: {LossAmount},nowTotalTrades:-{nowTotalTrades},- Datetime: {stockAlgoLogic.humanTime}\tStock: {stock}\tClose: {df_dict[stock].at[lastIndexTimeData, 'c']}")
                ...
                if lastIndexTimeData in df_dict[stock].index:
                    ...
                    if df_dict[stock].at[lastIndexTimeData, "rsi"] > 60 and (stock_openPnl.empty) and nowTotalTrades < TotalTradeCanCome:
                        ...
                        logger.info(f"nowTotalTrades:-{nowTotalTrades}, {TotalTradeCanCome},Entry- Datetime: {stockAlgoLogic.humanTime}\tStock: {stock}\tClose: {df_dict[stock].at[lastIndexTimeData, 'c']}")
